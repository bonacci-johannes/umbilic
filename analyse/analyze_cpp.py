...
# %%
t_start=50
corr_full = corr_full[:, :, t_start-1:]
time=time[t_start-1:]
# %%
corr_full_mean = numpy.mean(corr_full, axis=0)
corr_full_err = numpy.std(corr_full, axis=0) / numpy.sqrt(corr_full.shape[0])

# %%
sub_samples = 25
n_sub = samples_full // sub_samples
corr = numpy.zeros((n_sub, 2, len(time)))
...
for n in tqdm.tqdm(range(corr.shape[0])):
    for m in range(2):
        corr_csaps[n, m], z_csaps[n, m], _ = z_func_csaps(time=time, y=corr[n, m, :],
                                                          y_std_err=corr_std[m, :] / numpy.sqrt(n_sub),
                                                          sigma_weight=1,
                                                          z_0=3 / 2,
                                                          smooth=smooth)
...
# %%
smooth_full = smooth
corr_sm = numpy.zeros_like(z_mean)
z_sm = numpy.zeros_like(z_mean)
for m in range(2):
    corr_sm[m, :], z_sm[m, :], _ = z_func_csaps(time=time, y=corr_full_mean[m],
                                                y_std_err=corr_full_err[m],
                                                sigma_weight=1,
                                                z_0=3 / 2,
                                                smooth=smooth_full)
...
for m, color in zip([1,0], ['tab:red', 'tab:blue']):
    ...
    axs[1].fill_between(time[idx],
                        z_sm[m, idx] - 3 * z_std[m, idx] / numpy.sqrt(n_sub),
                        z_sm[m, idx] + 3 * z_std[m, idx] / numpy.sqrt(n_sub),
                        color=color, alpha=0.25)
    axs[1].plot(time[idx], z_sm[m, idx], color=color)
    #axs[1].plot(time[idx], z_mean[m, idx], color='k', linestyle='--')
...
plt.show()
# %%
fig.savefig('figures_temp/Fig_z_time.pdf', pad_inches=0, bbox_inches='tight')
